...
namespace Proline.Classic.LevelScripts.Mission
{
    public class HePacStandard : ScriptInstance
    {
        ...
        public override async Task Execute(params object[] args)
        {
            ...
            while (Stage != -1)
            {
                ...
                if (false)//IsPointWithinActivationRange())
                {
                    ...
                    if (API.GetInteriorFromEntity(CitizenFX.Core.Game.PlayerPed.Handle) != 0)
                    {
                        foreach (var securityPanel in _securityPanels)
                        {
                            if (CitizenFX.Core.Game.PlayerPed.IsNearEntity(securityPanel, new Vector3(1, 1, 1)))
                            {
                                Screen.DisplayHelpTextThisFrame("Press ~INPUT_CONTEXT~ to delete");
                                if (CitizenFX.Core.Game.IsControlJustPressed(0, Control.Context)) _vaultDoor.Delete();
                            }
                        }

                        foreach (var securityPanel in _securityPanels)
                            if (CitizenFX.Core.Game.PlayerPed.IsNearEntity(securityPanel, new Vector3(1f, 1f, 1f)))
                            {
                                Screen.DisplayHelpTextThisFrame("Press ~INPUT_CONTEXT~ to delete");
