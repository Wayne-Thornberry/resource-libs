...
namespace Proline.Classic.LevelScripts.Event
{
    public class ReArmouredTruck : ScriptInstance
    {
        ...
        private async Task NotifyPlayer()
        {
            ...
            CitizenFX.Core.Game.PlaySound("Friend_Deliver", "HUD_FRONTEND_MP_COLLECTABLE_SOUNDS");
        ...
        public override async Task Execute(params object[] args)
        {
            ...
            while (Stage != -1)
            {
                if (_armouredTruck.Exists())
                {
                    ...
                    {
                        if (CitizenFX.Core.Game.PlayerPed.CurrentVehicle == _armouredTruck)
                        {
                            if (_truckBlip.Alpha == 255)
                                _truckBlip.Alpha = 0;

                            Screen.DisplayHelpTextThisFrame("Stop the truck to spawn the loot");
                            if (CitizenFX.Core.Game.PlayerPed.CurrentVehicle.IsStopped)
                            {
                                Screen.DisplayHelpTextThisFrame("Press ~INPUT_CONTEXT~ to open the back of the truck");
                                if (CitizenFX.Core.Game.IsControlPressed(0, Control.Context))
                                {
                                    LootTruck();
                                    MarkTruckForDeletion();
                                    // TerminateScript();
        ...
        private void ArmouredTruckDestroyed(int arg1, int arg2, uint arg3, bool arg4, int arg5)
        {
            ...
            if (veh != _armouredTruck || CitizenFX.Core.Game.PlayerPed.Handle != arg2 || _hasLootSpawned || _hasDestroyed) return;
            _hasDestroyed = true;
        ...
        {
            var veh = Entity.FromHandle(arg1);
            if (veh != _armouredTruck || CitizenFX.Core.Game.PlayerPed.Handle != arg2 || _hasAttacked) return;
            _hasAttacked = true;
        ...
        {
            if (CitizenFX.Core.Game.Player.WantedLevel < wantedLevel)
                CitizenFX.Core.Game.Player.WantedLevel = wantedLevel;
        }
