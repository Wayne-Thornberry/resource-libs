...
namespace Proline.Classic.Engine.Components.CDebugInterface
{
    ...
    { 
        ...
        {
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_FONT, font);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_PROPORTIONAL, true);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_SCALE, 0.0f, scale);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_COLOUR, 255, 255, 255, 255);
            //// NativeAPI.SetTextDropshadow(0, 0, 0, 0, 255);
            //// NativeAPI.SetTextEdge(1, 0, 0, 0, 255);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_DROPSHADOW);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_OUTLINE);
            //NativeAPI.CallNativeAPI(Hash.SET_TEXT_CENTRE);
            //NativeAPI.CallNativeAPI(Hash.BEGIN_TEXT_COMMAND_PRINT, "STRING");
            //NativeAPI.CallNativeAPI(Hash.ADD_TEXT_COMPONENT_SUBSTRING_TEXT_LABEL, text);
            //NativeAPI.CallNativeAPI(Hash.END_TEXT_COMMAND_PRINT, 2000, 1);
            //NativeAPI.CallNativeAPI(Hash._DRAW_TEXT, vector2.X, vector2.Y);
        }
         

        public static void DrawDebugText3D(string text, Vector3 vector3, float scale2, int font)
        {
            //var camCoords = NativeAPI.CallNativeAPI<Vector3>(Hash.GET_GAMEPLAY_CAM_COORD);
            //var distance = NativeAPI.CallNativeAPI<float>(Hash.GET_DISTANCE_BETWEEN_COORDS, camCoords.X, camCoords.Y, camCoords.Z, vector3.X, vector3.Y, vector3.Z, false);
            //var scale = (1 / distance) * scale2;
            //var fov = (1 / NativeAPI.CallNativeAPI<int>(Hash.GET_GAMEPLAY_CAM_FOV)) * 75;
            //scale = scale * fov * 1f;
            //var f = new OutputArgument();
            //var g = new OutputArgument();
            //var j = new OutputArgument();

            //var x = NativeAPI.CallNativeAPI<bool>(Hash._WORLD3D_TO_SCREEN2D, vector3.X, vector3.Y, vector3.Z, f, g, j);
            //var d = new Vector3(f.GetResult<float>(), g.GetResult<float>(), (j).GetResult<float>());
