        ...
        public override async Task Execute(params object[] args)
        {
            while (true)
            {
                ...
                else if (Game.IsControlJustReleased(0, Control.CharacterWheel))
                {
                    ...
                    CitizenFX.Core.Native.API.SwitchOutPlayer(Game.PlayerPed.Handle, 1, 1);
                    await BaseScript.Delay(3000);
                    Game.PlayerPed.Position = new Vector3(new Random().Next(-4500, 4500), new Random().Next(-7500, 5500), World.GetGroundHeight(new Vector2(Game.PlayerPed.Position.X,
                    ...
                    await BaseScript.Delay(3000);
                    CitizenFX.Core.Native.API.SwitchInPlayer(Game.PlayerPed.Handle);
                    plySwitch = null;
                }
                ...
                if (Game.IsControlPressed(0, Control.CharacterWheel))
                {
                    ...
                    CitizenFX.Core.Native.API.HideHudAndRadarThisFrame();
                    ...
                    if(plySwitch.IsLoaded)
                        plySwitch.Render2DScreenSpace(new PointF(70f - (CitizenFX.Core.Native.API.GetSafeZoneSize() - 0.89f) / 0.11f * 78f, (Screen.Height - 150f) - (CitizenFX.Core.Native.API.GetSafeZoneSize() - 0.89f) / 0.11f * 78f), new PointF(150f, 150f));
