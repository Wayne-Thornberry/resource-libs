...
namespace Proline.Engine.LevelScripts.Mission
{
    public class HePacStandard : GameScript
    {
        ...
        {
            return CitizenFX.Core.Native.API.GetInteriorAtCoords(position.X, position.Y, position.Z);
        }
        ...
        {
            CitizenFX.Core.Native.API.DoorControl((uint)_vaultDoor.Model.Hash, _vaultDoor.Position.X, _vaultDoor.Position.Y,
        ...
        public override async Task Execute(params object[] args)
        {
            ...
            while (Stage != -1)
            {
                ...
                if (false)//IsPointWithinActivationRange())
                {
                    ...
                    if (CitizenFX.Core.Native.API.GetInteriorFromEntity(Game.PlayerPed.Handle) != 0)
                    {
                        ...
                        foreach (var securityPanel in _securityPanels)
                            if (Game.PlayerPed.IsNearEntity(securityPanel, new Vector3(1f, 1f, 1f)))
                            {
                                ...
                                if (Game.IsControlJustPressed(0, Control.Context))
                                {
                                    ...
                                    clos.Delete();
                                    CitizenFX.Core.Native.API.PlaySoundFrontend(-1, "alarm_loop", "dlc_xm_submarine_sounds", false);
                                }
                            }
                    }
                }
