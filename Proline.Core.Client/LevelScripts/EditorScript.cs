    ...
    public class EditorScript : LevelScript
    {
        ...
        public override async Task Execute(params object[] args)
        {
            ...
            while (Stage == 0)
            {
                ...
                foreach (var item in _garbage)
                {
                    ...
                    ExampleAPI.DrawEntityBoundingBox(item.Handle, 125, 0, 0, 100);
        ...
        private void EditorControls()
        {
            ...
            else if (Game.IsControlJustReleased(0, Control.Attack))
            {
                ...
                ExampleAPI.FindAllPeds(out var handles);
                han.AddRange(handles);
                ExampleAPI.FindAllPickups(out handles);
                han.AddRange(handles);
                ExampleAPI.FindAllProps(out handles);
                han.AddRange(handles);
                ExampleAPI.FindAllVehicles(out handles);
                han.AddRange(handles);
                ...
                {
                    var d = ExampleAPI.ScreenRelToWorld(_cam.Position, _cam.Rotation, new Vector2(_cx, _cy), out var dir);
                ...
                else
                {
                    ...
                    {
                        var raycast = World.Raycast(ExampleAPI.ScreenRelToWorld(_cam.Position, _cam.Rotation, _points[i], out var dir), dir, 1000f, IntersectOptions.Everything);
            ...
            else if (Game.IsControlPressed(0, Control.Attack))
            {
                ...
                if(_buttonHeld2 > 10)
                {
                    ExampleAPI.DrawDebug2DBox(_st, new PointF(_cx, _cy), Color.FromArgb(100, 100, 100, 100));
