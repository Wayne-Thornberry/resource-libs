...
namespace Proline.ExampleClient.Scripts.UI
{
    public class UIPlayerSwitch : DemandScript
    {
        private PlayerSwitch plySwitch;
        private int sel;
        private int oldSel;

        public UIPlayerSwitch(string name) : base(name)
        {
        ...
        public override async Task Execute(object[] args, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                else if (Game.IsControlJustReleased(0, Control.CharacterWheel))
                {
                    ...
                    LogDebug($"R: {R}, r {r}, Theta {theta}, CurrentPos {position} X {x}, Y {y}");
