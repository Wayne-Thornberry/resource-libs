...
namespace Proline.ClassicOnline.MScripting
{
    internal class LWScriptManager
    {
        ...
        {
            var container = GetScriptContainers(scriptName);
            return container.Length;
        }

        private ScriptContainer[] GetScriptContainers(string scriptName)
        {
            return _liveScripts.Where(e => e.ScriptName.Equals(scriptName)).ToArray();
        }
