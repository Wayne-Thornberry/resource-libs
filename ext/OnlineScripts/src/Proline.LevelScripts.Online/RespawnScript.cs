...
{
    public class RespawnScript : ScriptInstance
    {
        ...
        public void Update()
        {
            ...
            //                CountTimer = Tools.ConvertMsToFloat(DelayedRespawnTime);
        ...
        private void RespawnPlayer(Vector3 position)
        {
            ...
            CitizenFX.Core.Game.PlayerPed.Position = new Vector3(position.X, position.Y, World.GetGroundHeight(position));
        }
        ...
        private Vector3 GetPlayerRespawnLocation()
        {
            switch (RespawnType)
            {
                case SpawnType.NEAREST_SIDEWALK:
                    return World.GetNextPositionOnSidewalk(new Vector3(CitizenFX.Core.Game.PlayerPed.Position.X + DistanceRange,
        ...
        {
            CitizenFX.Core.Game.PlaySound("MP_Flash", "WastedSounds");
