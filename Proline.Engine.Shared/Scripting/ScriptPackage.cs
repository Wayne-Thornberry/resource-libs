...
namespace Proline.Engine.Scripting
{
    ...
    internal class ScriptPackage : EngineObject
    {
        ...
        internal InternalScript GetScriptWrapper(string scriptName)
        {
        ...
        private void Load()
        {
            ...
            foreach (var classPath in _scriptClasses)
            {
                ...
                var wrapper = InternalScript.CreateNewScript(scriptName, assembly, type, scriptConfig, _envType);
        ...
        internal IEnumerable<InternalScript> GetScriptWrappers()
        {
        ...
        internal static void UnregisterPackage(ScriptPackage sp)
        {
            ...
            foreach (var scriptName in sp.GetScriptWrappers())
            {
                try
                {
                    InternalScript.UnregisterScript(scriptName);
                }
        ...
        internal static void RegisterPackage(ScriptPackage sp)
        {
            ...
            foreach (var script in sp.GetScriptWrappers())
            {
                try
                {
                    InternalScript.RegisterScript(script);
                }
                catch (ArgumentException e)
                {
